"""
Implements m-bounded dyck-k languages sampling and writing to disk
"""
...
class DyckPDFA:
  ...
  def sample_stack_state(self, min_depth, min_open_slots):
    """
    For use in holding out states from training for evaluation.
    generates a valid stack state.

    Samples uniformly from the permissible stack states.

    Args:
      min_depth: minimum number of stack elts in generated state
      min_open_slots: minimum number of open slots in generated
                      stack state. (Distance from stack bound.)
    Returns:
      A list of characters 'a' and 'b' representing a stack state,
      e.g., ['a','b']
    """
    max_depth = self.max_stack_depth - min_open_slots
    depth = random.randint(min_depth, max_depth)
    random_state = (random.random() for x in range(depth))
    state = ['a' if r>=0.5 else 'b' for r in random_state]

    states_covered = 0.5 * 2**(self.max_stack_depth - depth + 1)
    max_possible_states = sum((self.args['language']['bracket_types']**i for i in range(self.args['language']['train_max_stack_depth']+1)))

    return state, states_covered/max_possible_states
  ...
  def get_token_distribution_for_state(self, state_vec):
    ...
    if len(state_vec) == 0:
      ...
      for id_str in self.ids:
        distrib_vec[self.vocab['(' + id_str]] = 1/len(self.ids)
      distrib_vec[self.vocab[state_vec[-1] + ')']] = 1
    return distrib_vec / torch.sum(distrib_vec)
    ...
    for i in tqdm(range(sample_count),smoothing=0.01):
      ...
      while not sample_met_constraints:
        ...
        if length >= min_length and length <= max_length:
          if not args['language'].get('length_effects', True):
            length = np.random.randint(min_length - 1, length)
